...
import { getFarmers, getDraftFarmers } from '@services/farmerService';
import { getVisits }    from '@services/visitService';
import { useToast }     from '@hooks/useToast';

const theme = getTheme('data_entry_operator');

// Daily capture goal — a fixed target, not measured data (no per-DEO quota
// configuration exists yet). Progress against it is computed from real counts.
...
export default function DEODashboardPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [farmersRes, draftsRes, visitsRes] = await Promise.all([
          getFarmers({ limit: 500 }),
          getDraftFarmers({ limit: 100 }),
          getVisits({ limit: 500 }),
        ]);
        if (cancelled) return;
        setFarmers(farmersRes.farmers);
        setDrafts(draftsRes.farmers);
        setVisits(visitsRes);
      } catch (err) {
  ...
  const capturedToday   = useMemo(() => farmers.filter(f => isToday(f.createdAt ?? f.created_at)).length, [farmers]);
  const capturedThisWeek = useMemo(
    () => farmers.filter(f => { const d = daysSince(f.createdAt ?? f.created_at); return d != null && d <= 7; }).length,
    [farmers]
  );
  const visitsToday = useMemo(() => visits.filter(v => isToday(v.createdAt)).length, [visits]);
  ...
  const quotaPct = Math.min(100, Math.round((capturedToday / DAILY_TARGET) * 100));
  ...
  return (
    <div className="space-y-6">
      ...
      <RoleHeroBanner
        ...
        stats={[
          { label: 'Captured Today', value: capturedToday },
          { label: 'Drafts Saved',   value: drafts.length },
          { label: 'Visits Logged',  value: visitsToday   },
        ]}
      />
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        {[
          { icon: 'fas fa-keyboard',      value: capturedToday,   label: 'Captured Today',  change: `${Math.max(0, DAILY_TARGET - capturedToday)} remaining`, up: true,  color: theme.accent },
          { icon: 'fas fa-file-pen',      value: drafts.length,   label: 'Drafts Pending',  change: drafts.length > 0 ? 'In progress' : null,                 up: false, color: '#d97706' },
          { icon: 'fas fa-calendar-check', value: visitsToday,    label: 'Visits Logged',   change: 'Today',                                                   up: true,  color: '#2563eb' },
          { icon: 'fas fa-check-double',  value: capturedThisWeek, label: 'Total This Week', change: null,                                                     up: true,  color: '#16a34a' },
              ...
              <div className="flex items-end justify-between">
                <div>
                  <div className="text-4xl font-extrabold text-foreground" style={{ color: theme.accent }}>
                    {capturedToday}
                    <span className="text-xl text-muted-foreground font-medium ml-1">/ {DAILY_TARGET}</span>
                  </div>
                  <div className="text-xs text-muted-foreground mt-1">farmers captured today</div>
                </div>
                <div className="text-right">
                  <div className="text-xs text-muted-foreground">Remaining</div>
                  <div className="text-2xl font-extrabold text-amber-600">{Math.max(0, DAILY_TARGET - capturedToday)}</div>
                </div>
              </div>
          ...
          <Card.Body className="p-0">
            {drafts.length === 0 ? (
              <div className="py-10 text-center text-muted-foreground text-xs">
                <i className="fas fa-circle-check mr-1.5" />No saved drafts — you're all caught up.
              </div>
            ) : (
                ...
                {drafts.map((d) => (
                  <div key={d.id} className="flex items-center gap-3 px-5 py-3.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-xs font-semibold text-foreground">{d.name || 'Unnamed draft'}</div>
      ...
      <Card>
        ...
        <Card.Body className="p-0">
          {recent.length === 0 ? (
            <div className="py-10 text-center text-muted-foreground text-xs">
              <i className="fas fa-users mr-1.5" />No farmers registered yet.
            </div>
          ) : (
